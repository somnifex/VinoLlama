...
- **Settings**: configure host, port, backend, generation defaults, theme, UI language, and local llama.cpp/OpenVINO deployment. The desktop app starts the local API automatically by default.
...
- End-user deployment manager for OpenVINO Runtime detection, llama.cpp candidate discovery, recommendations, and managed local deployment of validated llama-server binaries.
